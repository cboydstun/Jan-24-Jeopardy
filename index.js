...
//   let fetchResult = await fetch('./test.json');
//   return await fetchResult.json();
// }
// const test = await getQuestions();
...
const question1for200 = document.getElementById('question-1-200');
...
// question to ask, do the category and questions be in a getelementbyid or a queryselector? And are there to many elements called up top?
question1for200.addEventListener('click', (e) => {
  e.preventDefault();
  question1for200.textContent = 'surprise';
});
...
// guessButton AddEventListeners
guessButton.addEventListener('click', () => {
  const inputValue = document.querySelector('.answer').value;
  const questionValue = parseInt(inputValue.substring(1));
  if (playerTurn.textContent === "player one's turn") {
    player1Score += questionValue;
    player1.textContent = `Player-1 Score: ${player1Score}`;
  } else if (playerTurn.textContent === "player two's turn") {
    player2Score += questionValue;
    player2.textContent = `Player-2 Score: ${player2Score}`;
  } else {
    // Deduct points when the answer is incorrect
    if (playerTurn.textContent === "player one's turn") {
      player1Score -= questionValue;
      player1.textContent = `Player-1 Score: ${player1Score}`;
    } else {
